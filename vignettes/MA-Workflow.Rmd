...
carefully and in a non--technical way.
...
A section on how to interpret these two kind of plots has also  been added.
...
#In order to download the package from GitHub, we need the "install_github" function from the "remotes" packages. We download the latest developer version of "remotes" from GitHub as well with the devtool::install_github function; note that this is necessary as the current "remotes" version on CRAN doesn't currently allow us to correctly download the "maEndToEnd" package: 
...
For quality control, we take the the log2 of `Biobase::exprs(raw_data)`, 
...
5 arrays could be considered to be outliers: 2826_I, 2826_II, 3262_II, 3302_II and 
...
In comparison to the first PCA analysis before RMA, we see that now the first principal 
...
The outliers that could be found here are 2826_II, 3262_II, 3271_I, 2978_II and 3332_II (Figure \@ref(fig:alternativeHeatmapWithPatientProbesAsLabels)). 
2826_II, 3262_II and 3332_II were found to be outliers in both RLE and heatmap and could therefore considered to be removed; however, to stay as close as possible to the
...
variance is a mix of the median variance of all genes and the per gene variance.
...
which variables we want to consider. As we want to find differential expression 
between non--inflamed and inflamed tissue, those two are the only two variables 
...
and the columns are the variables we include into our linear model. 
For example, the first two rows of the design matrix `design_palmieri_CD` 
correspond to the two arrays for individual "164". 

The design matrix has the entries 0 and 1 and thus tells us 
...
i. e. each patient array. If we have a closer look at the single rows, we see that for 
each sample, there are two "ones" assigned: one to one of the variable "nI" or "I" 
corresponding to the tissue the probe came from, and one to the corresponding 
...
Note that the differences of the fitted expression values between inflamed-- and non--inflamed samples of one patient is the same for all patients and is determined by the difference between the variable coefficients of `I` (`r crat_coef["I"]`) and `nI` (`r crat_coef["nI"]`), which is  `r crat_coef["I"] - crat_coef["nI"]` (Figure \@ref(fig:3obtainFitCRAT)). 
...
The "difference" between non--inflamed and inflamed tissue of `r crat_coef["I"] - crat_coef["nI"]` actually 
is a log fold change, as our expression data is on the log scale. `r crat_coef["I"] - crat_coef["nI"]`
therefore is our log fold change for the CRAT gene.
...
We get a low p--value of 8e-06 and thus can conclude
...
highly expressed, so that it wasn't possible to find background--genes with an equally 
high expression.

The "all gene" curve has its maximum at the lowest mean expression of the three. This can be explained by a high number of lowly expressed genes in all samples. 
...
topGO requires a topGOdata object containing the necessary information for the analysis. We follow the steps described in the topGO vignettes: First, we will create a named vector `all_genes` with all genes to be analyzed, i.e. DE--genes and background genes: 
  ...
  3. We create the `all_genes` vector:
    a) First, we select all the elements from `inSelection` 
