...
```{r pkgList, results="hide", warnings=FALSE}
...
Furthermore, we specified our `AnnotatedDataFrame` "`SDRF`" created earlier from the SDRF 
file as `phenoData`. Thus, we had to make sure that we import the CEL files 
...
dominant source of variation. Note that the second principal components
...
We then create a table of `idx_man_threshold` to summarize the results,
...
We used the function `select`
from `r Biocpkg("AnnotationDbi") ` to query the gene symbols and associated
short descriptions for the transcript clusters. For each cluster, we added the 
gene symbol (`SYMBOL`) and a short description of the gene the cluster represents (`GENENAME`). 

In a second step, we filtered out the probes that don't map to a gene,
...
First, we grouped `anno_palmieri` for their PROBEID; that way, 
...
```
Recall that `fData` enables us to access the feature data of an expression set. Until now, no feature data whatsoever is stored in the `fData(palmieri_final)`. Only 
...
We applied the empirical Bayes variance moderation method to it 
via the `eBayes()` function, which
computes moderated \(t\)--statistics: As in microarray analysis, 
...
expression with `genefinder`. The `genefinder` function returns a list with two elements
...
The following steps were carried through: 
  ...
  enrichment 
  analysis. We got a vector `in_universe` with the length of `gene_IDs` that has the 
