...
```{r arrayQualityMetricsRaw, eval = TRUE}
...
RLE_data_gathered <- 
  tidyr::gather(RLE_data, patient_probe, log2_expression_variation)

ggplot2::ggplot(RLE_data_gathered, aes(patient_probe, log2_expression_variation)) 
...
for (i in 1:length(Biobase::pData(palmieri_eset_norm)$Factor.Value.phenotype.)){
  if(grepl 
     ("non", 
       toString(Biobase::pData(palmieri_eset_norm)$Factor.Value.phenotype.[i])))
  ...
  if (grepl 
      ("Crohn", 
        toString(Biobase::pData(palmieri_eset_norm)$Factor.Value.disease.[i])))
    {disease_name="CD"}
...
row_names_for_heatmap <- 
  paste(row_names_dataframe$Phenotype, row_names_dataframe$Disease, sep="   ")
...
no_of_samples <- 
  table(paste0(Biobase::pData(palmieri_eset_norm)$Factor.Value.disease., "_", 
                  Biobase::pData(palmieri_eset_norm)$Factor.Value.phenotype.))
no_of_samples 
...
tissue <- 
  str_replace_all(Biobase::pData(palmieri_final)$Factor.Value.phenotype., " ", "_")
tissue <- 
  ifelse(tissue == "non-inflamed_colonic_mucosa", "nI", "I")

disease <- 
  str_replace_all(Biobase::pData(palmieri_final)$Factor.Value.disease., " ", "_")
disease <- 
  ifelse(disease == "Crohn's_disease", "CD", "UC")
