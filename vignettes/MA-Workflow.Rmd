...
Note that the differences of the fitted expression values between inflamed-- and non--inflamed samples of one patient always are the same for all patients and is determined by the difference between the variable coefficients of `I` (6.77) and `nI` (7.19), which is -0.425. This is the case because the same sample variable is activated by the design matrix for both samples from one patient, and thus only corrects the fitted expression values towards the original ones measured from the patients. 
...
res_t
#andere df als beim linearen Modell, deswegen anderer p-Wert
...
<!-- SR: what does this actually mean? That the values of the slopes/differences between I/nI are reasonably close to each other? -->
...
#SR: discuss whether this can be called fold-change or shouldn't rather be described as a difference #https://en.wikipedia.org/wiki/Fold_change ; eig log fold change 
...
<!-- SR: it would be really interesting to know now how significance of the difference was tested --> 
...
When comparing the "background gene" curve to the "foreground gene" curve, we see a similar curve form, indicating a sensible background matching. Note that the right-shift of the "foreground-gene" curve in comparison to the "background-gene" curve indicates that DE--genes are generally very highly expressed, so that it wasn't possible to find background--genes with an equally high expression.
...
all_genes <- factor(as.integer(inSelection[in_universe])) #SR: array mit Eintrag 1 falls TRUE und 0 falls FALSE
