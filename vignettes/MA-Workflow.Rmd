...
<!-- devtools::install_github("Steffireise/maEndToEnd", ref = "first_changes_steffi") -->
...
<!-- rmarkdown::render("MA-Workflow.Rmd",
output_format = BiocWorkflowTools::f1000_article(keep_tex = TRUE)) -->
...
# Download of the raw data from ArrayExpress
...
## Download of the raw data and the annotation data 
...
We now import the SDRF file with the `read.delim` function from the raw data folder in order to obtain the sample annotation.
...
* **metaData**
      + **phenoData**: A description of the samples in the experiment
      + **featureData**: metadata about the features on the chip or technology used for the experiment 
...
illustrated in Figure \@ref(fig:sumexp).



```{r sumexpOld, echo=FALSE, fig.show="asis", out.width="250px"}
# par(mar=c(0,0,0,0))
...
```{r sumexp, fig.cap = "Structure of Bioconductor’s ExpressionSet class.", echo=FALSE, fig.show="asis", out.width="250px"}
par(mar=c(0,0,0,0))
...
We now inspect the raw data a bit.

The pData function of the Biobase package directly accesses the "data" subfolder in the phenoData tab in the ExpressionSet `raw_data`. With the `head()` function, we can view the first six lines of the table. We have a look at the columns included and retain only those columns that are related to the experimental factors of interest.
...
head(Biobase::exprs(raw_data))

```
For quality analysis, we take the log2 of `Biobase::exprs(raw_data)`, as expression data is always analyzed on a logarithmic scale.
We then perform a principal component analysis (PCA) and plot it (see Figure \@ref(fig:qualityControlRawDataPCA)). Every point in the plot represents one sample, with the colour indicating the mucosa type (inflamed vs non-inflamed) and the shape indicating the disease (UC or CD).
We also plot a boxplot graph with boxes for the probe intensities on individual microarrays (Figure \@ref(fig:qualityControlRawDataBox)). Note that the `oligo::boxplot` function, i.e. the boxplot function of the oligo package, can take whole expression sets as argument. It accesses the expression data and performs a log2-transformation by default. We therefore can use `raw_data` as argument here.


<!--
# ```{r qualityControlRawDataPCAold, fig.cap="PCA plot of the log–transformed raw data."}
...
(qplot(PC1, PC2, data = dataGG, 
       ...
       asp = sd_ratio) #check if the ratio is right in the graph
     + scale_colour_brewer(palette = "Set2"))
...
The PCA (see Figure \@ref(fig:qualityControlRawDataPCA)) (performed on the log--intensity scale) plot of the raw data shows that the first principal component differentiates between the diseases. This means that the expression data mainly differs between the diseases. This might hinder our analysis, as we want to analyze the differential expression between inflamed- and non-inflamed tissue, independently of the disease a person has. When looking at the boxplot (Figure \@ref(fig:qualityControlRawDataBox)), we see that the intensity distributions of the individual arrays are quite different, indicating the need for an appropriate normalization, which we will discuss next.
...
are two levels of summarization to get to the gene level. The "probeset" summarization leads to the exon level. The gene/transcript level is given by "transcript clusters". Hence, the appropriate annotation package for our chip type is called
...
Before calibrating and evaluating the data, we want to perform another means of quality control, namly Relative Log Expression (RLE), as described in the article by Gandolfo et al [@Gandolfo_2018]. To this goal, we first perform an RMA without normalization: 
...
```
-->
```{r boxplotDataForRLETidy, fig.cap="Boxplot for the RLE values"}
...
RLE_data_gathered <- tidyr::gather(RLE_data, patient_probe, log2_expression_variation)

ggplot2::ggplot(RLE_data_gathered, aes(patient_probe, log2_expression_variation)) + geom_boxplot(outlier.shape=NA) + ylim(c(-2, 2)) + theme(axis.text.x= element_text(colour="aquamarine4", angle=60, size=6.5, hjust=1, face="bold"))
```

When looking at the boxplot in Figure \@ref(fig:boxplotDataForRLETidy), 5 arrays could be considered to be outliers: 2826_I, 2826_II, 3262_II, 3302_II and 3332_II are negatively y--shifted. We will keep these samples in mind for heatmap cluster analysis later on in the workflow. Arrays that are confirmed to be outliers by heatmap analysis can be considered to be removed for subsequent analysis.
...
<!--
# ```{r PCAMetricsCalibrated2, fig.cap = "PCA plot of the calibrated data.", eval = TRUE }
...
(qplot(PC1, PC2, data = dataGG, 
       color =  Phenotype, 
       shape = Disease,
       main = "PCA plot of the calibrated data", 
       size = I(2), 
       xlab = paste0 ("PC1, VarExp: ", percentVar[1], "%"),
       ylab = paste0 ("PC2, VarExp: ", percentVar[2], "%"),
       asp = sd_ratio)
...
In comparison to the first PCA analysis before RMA, we see that now the first principal component clusters between tissues types (see Figure \@ref(fig:PCAMetricsCalibrated). This indicates that now differential expression between the tissue types can be analyzed quite well.
...
```{r rownamesForHeatmap, fig.height = 8.5, eval = TRUE, echo=TRUE}
phenotype_names <- c()
disease_names <- c()
...
pheatmap(dists, col = (hmcol), 
         ...
         legend_labels=(c("low difference", "high difference")),
         main = "Distance clustering heatmap for the calibrated samples")

```

On the heatmap plot (Figure \@ref(fig:clusteringHeatmap)) we also see that the samples do not cluster strongly by tissue, confirming the impression from the PCA plot that the separation
between the tissues is not perfect. The purple stripes in the heatmap might correspond
to outliers that could potentially be removed. We produce the same heatmap again with the sample identifiers as row names in order to see whether the outliers in the heatmap are the same ones as the outliers in the RLE plot beforehand: 


```{r alternativeHeatmapWithPatientProbesAsLabels, fig.cap="Heatmap with sample IDs", fig.height = 8.5, eval = TRUE}
...
pheatmap(dists, col = (hmcol), legend=TRUE, 
         legend_breaks=c(min(dists, na.rm=TRUE), 
                         max(dists, na.rm=TRUE)), 
         legend_labels=(c("low difference", "high difference")),
         main = "Distance clustering heatmap for the calibrated samples")
...
The outliers that could be found here are 2826_II, 3262_II, 3271_I, 2978_II and 3332_II (see Figure \@ref(fig:alternativeHeatmapWithPatientProbesAsLabels)). 
2826_II, 3262_II and 3332_II were found to be outliers in both RLE and heatmap and could therefore considered to be removed; however, to stay as close as possible to the paper results we want to compare our results to, we continue with the complete set of samples.
...
We calculate the row--wise medians from the expression data, as they represent the transcript medians, and assign them to `palmieri_medians`. We plot a histogram: 
...
In the histogram of the gene--wise medians (see Figure \@ref(fig:IntensityBasedManualFiltering)), we 
can clearly see an enrichment of low medians on the left hand side. 
These represent the genes we want to filter. In order to infer a cutoff from the data, we inspect the histogram. We visually set a cutoff line `man_threshold` to the left of the histogram peak in order to not exclude too many genes. Transcripts that don't have intensities over the threshold in at least as many arrays as the smallest experimental group are excluded. 
...
When we are content with the manual threshold, we filter out all transcripts that don't have intensities over the threshold in at least as many arrays as the smallest experimental group, represented by `min(no_of_samples)`, which we define as `samples_cutoff`. We write a function `idx_man_threshold` that is applied to each row, i.e. to each transcript across all arrays. It evaluates whether the number of entries that is bigger than man_threshold (`sum(x > man_threshold)`) is bigger than the `samples_cutoff` and gives out TRUE or FALSE for each row, i.e. each transcript. 
...
In the last step, we subset our expression set to `palmieri_manfiltered` and keep the TRUE elements of `idx_man_threshold`.

```{r filteringOfLowIntensity_transcripts}
samples_cutoff <- min(no_of_samples)

idx_man_threshold <- apply(Biobase::exprs(palmieri_eset_norm), 1, function(x){
...
gene symbol (`SYMBOL`) and a short description of the gene the cluster represents (`GENENAME`). 

In a second step, we filter out the probes that don't map to a gene, i.e. that don't have a gene symbol assigned. 
...
We compute a summary table in the code below to see how many there are. First, we group `anno_palmieri` for their PROBEID; that way, the subsequent operations aren't carried through for each single row, but for each group, i.e. each PROBEID.
We then summarize the groups and indicate the number of different genes assigned to a probe in the column `no_of_matches`. 
Now, we filter for PROBEIDs with multiple matches, i.e. `no_of_matches > 1`.
...
By left--joining with `anno_palmieri`, we thus add the "SYMBOL" and "GENENAME" columns from `anno_palmieri for only the PROBEIDs that have been in `fData(palmieri_final)` and thus get the feature Data for the filtered probes.
...
In order to analyse which genes are differentially expressed in inflamed-- and non--inflamed tissue, we have to fit a linear model to our expression data.
Linear models are the "workhorse" for the analysis of experimental
data. They can be used to analyse almost arbitrarily complex designs, however
they also take a while to learn and understand and a thorough description
is beyond the scope of this workflow.

Mike Love's and Michael Irizzary's [genomics class](http://genomicsclass.github.io/book/)
is a very good resource, especially its section on [interactions and contrasts](http://genomicsclass.github.io/book/pages/interactions_and_contrasts.html).
It might also be helpful to learn a bit of linear algebra to understand
...
across genes for the residual variance estimation leading to "moderated" \(t\)--statistics, 
and stabilizing the analysis for experiments 
with just a small number of arrays [@Smyth_2004]. Conceptually, the final per gene variance
is a mix of the median variance of all genes and the per gene variance.
...
For building our linear model, we have to think about which variables we are interested in. As we want to find differential expression in non--inflamed-- vs inflamed tissue, those two are the only variables we would have to include. 
However, since we have two arrays per individual patient, we have a "Paired Samples" design
(see section 9.4 of the user guide). 
Thus, the first factor we need is a blocking factor for the individuals that will absorb differences between
them. Therefore, we block on patients, which means that the patient IDs become variables of the linear model.

Then we create factors that give us the grouping for the diseases and
the tissue types (non-inflamed and inflamed). We furthermore simplify the names of the
diseases to UC and DC and the names of the tissue types to nI and I.
...
In the design matrix, the rows represent the patient probe, and the columns are the variables we include into our linear model. For example, the first two rows of the design matrix `design_palmieri_CD` 
correspond to the two arrays for individual "164". 
The design matrix has the entries 0 and 1 and thus tells us which variables are "active" for which sample: 
it can be seen as a switch that turns the single variables "on" (with a 1 at the corresponding position) or "off" (with a 0 at the corresponding position) for each row, i. e. each patient probe. If we have a closer look at the single rows, we see that for each sample, there are two "ones" assigned: one to one of the variable "nI" or "I" corresponding to the tissue the probe came from, and one to the corresponding patient--specific variable.
...
Before heading on to find all differentially expressed genes for both diseases, we will first have a look on how this works in principle for one gene. We will fit the linear model for one gene and run a t--test in order to see whether the gene is differentially expressed or not.

## Analysis of differential expression on the basis of one gene

For linear model fitting and subsequent testing for differential expression by t-test, we will pick the gene with the PROBEID 8164535. It is the *CRAT* gene and will be named as such in the following code. 
...
<!--```{r visualizeExpressionChangesOld}
...
#crat_data <- as.data.frame(crat_fitted) 
#crat_data <- tibble::rownames_to_column(crat_data, var = "sample_id")
#crat_data <- mutate(crat_data, org_value = crat_expr,
#                               tissue = tissue[disease == "CD"],
#                               individual = i_CD)
...
We see that overall, this gene is expressed lower in inflamed tissue ((see Figure \@ref(fig:visualizeExpressionChanges)). We also see that the absolute expression intensities vary a lot between patients However, we have already taken care of this problem by introducing blocking factors on the individuals, which allows us to fit one line to each individual; if we hadn't blocked for individuals, the linear model would fit one single line to all of these lines, resulting in a very high variance.

We now generate the variable coefficients by fitting a linear model. We get a vector `crat_coef` with one entry for each variable.
...
```{r}
#lin.fit <- lmFit(palmieri_final[,disease == "CD"],
#                         design = design_palmieri_CD)

#crat_all <- eBayes(lmFit(palmieri_final[,disease == "CD"],
#             design = design_palmieri_CD))



#ordinary.t <- lin.fit$coef[,2] / lin.fit$stdev.unscaled[,2] / lin.fit$sigma
#ordinary.t["8164535"]

#ordinary.t2 <- crat_all$coef[,2] / crat_all$stdev.unscaled[,2] / crat_all$sigma
#ordinary.t2["8164535"]

#ordinary.t <- fit$coef[,2] / fit$stdev.unscaled[,2] / fit$sigma --> mit contrast_fit ohne eBayes, also palmieri_fit_CD

#ordinary.t und ordinary.t2 geben das selbe Ergebnis: 57.9
```

In order to now obtain the expression values fitted by the model, we multiply the design matrix with this vector of coefficients `crat_coef`. Recall that for every row in the design matrix (i.e. every patient sample) only the variables with a 1 in the design matrix are taken into account for calculating the fitted expression value. 
This means that as output of the multiplication, we get a vector `crat_fitted` whose entries are the sum of relevant variable coefficients for each sample, respectively.
For example, the fitted value for patient sample `2114_I.CEL` is 6.97: it is the sum of the respective activated variable coefficients "nI" (7.1917 ) and "i_CD2114" (- 0.2215). 
...
Note that the differences of the fitted expression values between inflamed-- and non--inflamed samples of one patient always is the same for all patients and is determined by the difference between the variable coefficients of `I` (6.77) and `nI` (7.19), which is - 0.425 ((see Figure \@ref(fig:3obtainFitCRAT)). This is the case because the same sample variable is activated by the design matrix for both samples from one patient, and thus only corrects the fitted expression values in y--direction towards the original ones measured from the patients. 
This "difference" between non--inflamed and inflamed tissue of - 0.425 actually is a log fold change, as our expression data is on the log scale. - 0.425 therefore is our log fold change. 

### Differential expression analysis of the CRAT gene

In order to test whether the gene is differentially expressed or not, a \(t\)--test with the null hypothesis that there is no difference in the expression between non--inflamed and inflamed tissue is carried through. A look at the formula of the \(t\)--test shows us that the \(t\)--value depends on the variance of the sample: 

$$
 t = \frac{\bar{x} - \mu}{s/\sqrt{n}}
$$

We thus have made our \(t\)--test more sensible by reducing the variance by blocking on individuals.
...
We get a p--value of 8e-06 and thus have found that the *CRAT* gene is differentially expressed between non--inflamed and inflamed tissue. Note that the p--value isn't exactly the same one as below when analyzing the differential expression of all genes. The reason is a variance correction called "empirical Bayes" which we will implement later, leading to slightly different p--values. The method will be explained more in detail when applied.

## Contrasts and hypotheses tests

We now fit the linear model across all genes and
...
Thus, we create a contrast matrix consisting of only one contrast "I-nI": `r Biocpkg("limma")` 's function
`makeContrasts` creates this matrix from a symbolic description of the contrast of
interest.
We now fit a linear model to our data and apply the `contrasts.fit()`function to it in order to find genes with significantly differential expression between non--inflamed and inflamed tissue. We apply the "empirical Bayes" method to it with the `eBayes()` function, so that it computes moderated \(t\)--statistics: As in microarray analysis, the number of arrays often is quite small, only a small sample number can be used to calculate expression values of a certain gene. This might lead to variance values that aren't representative for the population. "Empirical Bayes" assumes that expression variance should be the same for all genes, and therefore shrinks the gene-wise residual sample variances towards a common value, borrowing variance information from other genes. 
...
Results can be extracted by use of the `topTable` function. We extract the results for both Crohn's disease and ulcerative colitis, and the results are sorted by their absolute \(t\)--statistics.
As a diagnostic check, we also plot the p--value histogram: We expect a uniform distribution for the
 p--values that correspond to true null hypotheses, while a peak near zero shows an enrichment for low p--values corresponding to differentially expressed (DE) genes. Note that if the p--value distribution for a dataset looks a lot different than the ones in the histograms below, this might lead to quality loss in the subsequent analysis. Reasons for a divergent p--value--distribution might be batch effects or a lack of consideration of other blocking factors in the design matrix beforehand. Thus, try to include all blocking factors and run the analysis again. If the problem previals, good resources to understand this phenomenon and its implications is the article on false discovery rate estimation by Korbininan Strimmer [@Strimmer_2008] and chapter 1--6 of Efron's book on Large--Scale Inference [@Efron_2010].
...
```
<!--
```{r extractResultsCDWithoutBlocking}
# 
# i_CD <- individual[disease == "CD"]
# design_palmieri_CD_without_ID <- model.matrix(~ 0 + tissue[disease == "CD"])
# colnames(design_palmieri_CD_without_ID)[1:2] <- c("I", "nI")
# rownames(design_palmieri_CD_without_ID) <- i_CD 
# 
# palmieri_fit_CD_without_ID <- eBayes(contrasts.fit(lmFit(palmieri_final[,disease == "CD"],
#                                 design = design_palmieri_CD_without_ID),
#                                 contrast_matrix_CD[1:2,]))
# table_CD_without_ID <- topTable(palmieri_fit_CD_without_ID, number = Inf)
# head(table_CD_without_ID)
# 
# 
# hist(table_CD_without_ID$P.Value, col = brewer.pal(3, name = "Set2")[1],
#      main = "inflamed vs non-inflamed - Crohn's disease", xlab = "p-values")
# 
...
## Comparison to the paper results
 ...
 When exploring the output of the topTable() function, we can see that there is a column called "P.Value" and one called "adj.P.Val". The latter describes the BH-corrected p-value that takes into account sample size and allows to control for the FDR (false discovery rate), while the former is just the "normal" p-value.
A p--value of less than 0.001 was used in the original paper as a significance
cutoff. However, it is highly recommended to use the adjusted p--value and thus control the FDR instead of using the "normal" p-value:
The FDR describes the percentage of false positives among all positive test results, while the p-value describes the probability that a certain event occurs under the assumption that the null hypothesis H0 is true; in a setup where all samples fulfill H0, p is therefore the probability to get a false positive result and thus describes the rate of false positives among all samples. H0 in our case is that a gene is not differentially expressed between inflamed and non-inflamed tissue. Even if we didn't have any differential expression among all genes, we would still expect a percentage of p false positives. This means that the absolute number of false positives would increase with the sample size. This problem is called "multiple testing problem", and occures a lot with biological data. In order to avoid the multiple testing problem, one should control for the FDR instead of the p-value: this way, we don't get more positives merely because of increasing sample sizes, and the share of false positives among all positives doesn't increase but stays below the FDR.

Nevertheless, in order to be able to compare our workflow results to the paper results, for now we also set a p-value cutoff at 0.001, as done in the paper.

The paper results can be downloaded as excel files from
 -[http://links.lww.com/IBD/A795](http://links.lww.com/IBD/A795) and should be saved as an .xlsx file named `palmieri_DE_res.xlsx` in your working directory. 
 
The paper results are given as identified differentially expressed genes
with a p--value less than 0.001. The corresponding FDRs are also listed in the results file and are 0.05 in Crohn's disease and 0.02 in ulcerative colitis. There are four tables in total, giving
the list of up and downregulated genes in CD and UC, respectively.
We calculate the overlap between our results and the ones from the paper as the ratio between the genes that were found both in our analysis as well as in the paper and the genes that were only found in the paper. We also calculate the number of total genes we find in our workflow analysis.
...
We find `r total_genenumber_CD` (CD) and `r total_genenumber_UC` (UC) differentially expressed genes ("DE--genes").
In the paper, 298 (CD) and 520 (UC) DE--genes were found for the two diseases at the same cutoff. This 
...
`r overlap_UC` for UC, showing us that roughly the same genes were found. 



<!--
The first plot we prodcue is a scatterplot that compares
the fitted (as returned by the linear model) to the actual values.
...
#crat_expr <- exprs(palmieri_final)["8164535", disease == "CD"]
#crat_coef <- eBayes(lmFit(palmieri_final[,disease == "CD"],
#                         design = design_palmieri_CD))$coefficients["8164535",]
...
#crat_data$tissue <- factor(crat_data$tissue, levels = c("nI", "I"))
#```


# ```{r scatterFittedVsActual}
#ggplot(data = crat_data, aes(x = org_value, y = fitted_value, 
#                              color = individual)) +
#      geom_abline(slope = 1, intercept = 0) +
#      geom_point() +
#      coord_fixed() +
#      ggtitle("Actual expression vs fitted values")
```

We see that the actual expression values are reasonably similar to
the fitted ones. They scatter around a 45° degree line.


<!--

# ```{r visualizeExpressionChangesOld2}


#ggplot(data = crat_data, aes(x = tissue, y = fitted_value, 
#                             group = individual, color = individual)) +
#      geom_line() +
#      ggtitle("Expression changes for the CRAT gene")
...
```
We can see that indeed the expression for the CRAT gene is
indeed reduced in inflamed tissue 
for most indiviudals (Although not for all of them,
2209 is a notable outlier here).

There is substantial between-subject variation here, as indicated
by the quite different line intercepts. Thus, blocking for every 
individual is indeed warranted to capture the relatively similar
fold changes between the different tissues within each subject.

We now vizualize the individual fold changes.

 ```{r visualizeIndividualFoldChanges}
#fcs <-  crat_data %>%
#        group_by(individual) %>%
#        summarize(fc = diff(org_value))

#summarize(group_by(crat_data, individual), fc = diff(org_value))

#ggplot(data = fcs, aes(x = "individual fold changes", y = fc, color = individual)) +
#      geom_jitter(height = 0, width = 0.1) +
#      ggtitle("Fold changes for the CRAT gene") +
#      geom_segment(aes(x = 0.8, y = -0.425, xend = 1.2, 
#                       yend = -0.425), colour = I("coral3")
#                   )

```

We see that the fold changes within each individual do indeed scatter around
the fitted value of -0.425.
-->

# Gene ontology (GO) based enrichment analysis
As mentioned before, it is recommended to use an FDR cutoff rather than a p--value cutoff, since this way you control an explicitly defined error rate and the results are easier to interpret and
to compare. For the following enrichment analysis, we create tables with differentially expressed genes for CD and UC, respectively, and choose an FDR cutoff of 10\%. Here, we focus on the CD subset of the data.
...
a bit better by performing a GO enrichment analysis. Essentially the
gene ontology ([http://www.geneontology.org/](http://www.geneontology.org/)) is 
a hierarchically organized collection of functional gene sets [@Ashburner_2000; @GO_2015; @du_Plessis_2011]. 
...
expression with `genefinder`. The `genefinder` function gives out two lists for each gene for which background genes should be found: one with the indices of the background genes found and one with the distances to the DE--genes. We have to extract the PROBEIDs, which correspond to the indices. We do that by using the `sapply` function, which gives us a single table with the DE--genes as column names and the PROBEIDs of the corresponding background genes in the cells below. 

We then create a vector `back_genes` containing all background gene PROBEIDs.

In order to eliminate foreground genes, i.e. DE--genes, from the `back_genes` set, we use the `setdiff` function. It returns all elements from the first argument (`back_genes`) that are not part of the second argument (`DE_genes_CD`). With the `intersect` function, we verify what we did: it should return 0, as there shouldn't be any intersect anymore between `back_genes`and `DE_genes_CD`. 

We create a multidensity plot with mean expression on the x-axis and curves for all genes, foreground genes and background genes, respectively (see Figure \@ref(fig:multidensityPlot)). We want to see whether the background genes show a plot similar to the foreground genes so that the background is not biased for the gene enrichment analysis. 
...
When comparing the "background gene" curve to the "foreground gene" curve, we see a similar curve form, indicating a sensible background matching (see Figure \@ref(fig:multidensityPlot)). Note that the right-shift of the "foreground-gene" curve in comparison to the "background-gene" curve indicates that DE--genes are generally very highly expressed, so that it wasn't possible to find background--genes with an equally high expression.
The "all gene" curve has its maximum at the lowest mean expression of the three. This can be explained by a high number of lowly expressed genes among all samples. 

For the actual testing of which GO gene sets are enriched in inflamed tissue, we use the `r Biocpkg("topGO")` package which implements a nice interface to Fisher testing and also has additional algorithms
...
topGO requires a topGOdata object containing the necessary information for the analysis. We follow the steps described in the topGO vignettes: First, we will create a named vector `all_genes` with all genes to be analyzed, i.e. DE--genes and background genes: 
  ...
  1. we create an `in_universe` vector by using the `%in%` matching function. We want to know which elements from `gene_IDs` are also contained in `DE_genes_CD` and `back_genes`, as the latter two are our gene universe we use for enrichment analysis. We get a vector `in_universe` with the length of `gene_IDs` that has the entry `TRUE` when the corresponding gene in `gene_IDs`could be also found in `DE_genes_CD` or `back_genes`, and `FALSE` otherwise.
  2. We do the same for our DE--genes and call this vector `inSelection`.
  3. We create the `all_genes` vector:
    a) First, we select all the elements from `inSelection` that are `TRUE` in `in_universe` by applying `all_genes <- inSelection[in_universe]`.
    b) Then, we convert the elements in `all_genes` from `TRUE` and `FALSE` to 0 and 1 by converting the vector to an integer vector. This way, each element in the vector is a 0 if the corresponding gene is a background gene and a 1 if the corresponding gene is a DE--gene. Also, we convert the vector to a factor.
    c) We name the vector elements with the corresponding `gene_IDs`.
...
all_genes <- inSelection[in_universe]
...
GO and tries to "decorrelate" it in order to report the most specific significant 
...
gives genes annotated to them; the significant ones are denoted with a "2" in the "raw p-value" column, the non--significant ones with a "1". We therefore select `raw p-value == 2`. Note that we don't get the actual p--values here because our `all_genes` vector doesn't contain this information; it only tells us whether a gene is differentially expressed or not.
...
```{r graphOfResults, fig.height = 6, eval=TRUE, fig.cap = "A graphical representation of the topGO results.", results='hide', dpi=300, cache=TRUE, out.width="400px"}
...
immune response show up as significant ((see \@ref(fig:graphOfResults)))
...
[Reactome](http://www.reactome.org/s) pathway database [@Croft_2013; @Fabregat_2015]. 
...
This will create a named vector that maps the PROBEIDs to the entrez ones, with the PROBEIDs as names and the entrez ids as vector elements.
...
with a p--value below the specified cutoff (see Figure \@ref(fig:reactomeBar)).
...
edge--thickness (that is the line connecting two pathways) is proportional to the number of overlapping genes between two pathways.
...
Again, the graph in Figure \@ref(fig:emapplot) shows pathways related to signalling and immune response. 
