...
We then perform a principal component analysis (PCA) and plot it (see Figure 
...
major the disease type is a major driver of gene expression differences.
...
control procedure, namly Relative Log Expression (RLE), as described in 
...
returns out TRUE or FALSE for each row, i.e. each transcript.

We than create a table of `idx_man_threshold` in order to summarize the results,
...
We apply the "empirical Bayes" method to it via the `eBayes()` function, which
computes computes moderated \(t\)--statistics: In microarray analysis, 
