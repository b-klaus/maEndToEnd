...
to compare. In the following enrichment analysis, we choose an FDR cutoff of 10\%.
...
perform the actual testing. <!-- SR: discuss "sensible result" -->  For this purpose we use the `r Biocpkg("topGO") ` which
...
all_genes <- factor(as.integer(inSelection[in_universe])) #SR: array mit Eintrag 1 falls TRUE und 0 falls FALSE; aber ist inSelection[in_universe] nicht exakt dasselbe wie inSelection? (setequal(inSelection, inSelection[in_universe])=TRUE)
names(all_genes) <- gene_IDs[in_universe] 
#SR: all_genes is prepared as described in the instruction  by Alexa on https://bioconductor.org/packages/release/bioc/vignettes/topGO/inst/doc/topGO.pdf, 4.4 
```
...
for details see the paper <!-- SR: Verweis auf Paper! --> or the package vignette.
...
#bedeutet raw p-value == 2, dass das die Gruppe der p-values ist, die signifikant sind? --> 2 sind die differential expressed genes, 1 die nicht
