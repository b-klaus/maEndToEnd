...
data. They can be used to analyse almost arbitrarly complex designs, however
...
Since we have two arrays per individual patient, we have a "Paired Samples" design
(see section 9.4 of the user guide). Thus, the first factor we need
...
diseases to UC and DC, respectively. 
...
In the actual model fitting every "1" in the model matrix corresponds to
a fitted model coefficient that represents a fitted mean or fold change,
whild the rows of the model matrix correspond to the different individuals.

For example, the first two rows of the model matrix `design_palmieri_CD` 
correspond to the two arrays for individual "164".

The first two columns, "I", and "nI" correspond to mean expression in inflammed
/ non--inflamed tissue respectively, while all the other columns correspond to
the "blocking" factors for all the other individuals. Here, individual "164"
corresponds to a baseline, so there are only blocking factors that adjust
the mean expression values for the other individuals. For example, individual
"183" has two ones in the first blocking factor column `i_CD183` corresponding
to the change in mean expression compared to individual "164".

Before heading on to find all differentially expressed genes for both diseases, we will first have a look on how this works in principle for one gene. We will fit the linear model for one gene and run a t--test in order to see whether it is differentially expressed or not.

# Illustration of the fitted linear models

We pick one of the PROBEIDs in the vector of differentially expressed genes `DE_genes_CD`and have a closer look at the linear model fitted to this gene. 
We choose the **CRAT** gene (probe ID 8164535) in CD. 
...
ggplot(data = crat_data, aes(x = tissue, y = org_value, 
...
We see that overall, this gene is expressed lower in inflamed tissue. For building our linear model, we have to think about which variables we are interested in. As we want to find differential expression in non--inflamed-- vs inflamed tissue, those two are the only variables we would have to include. 
However, we see that the absolute expression intensities vary a lot between patients. Therefore, we block on patients, i.e. the patient IDs become variables of the linear model. The design matrix has the different patient samples in its rows and the linear model variables in its columns. It has the entries 0 and 1 and thus tells us which variables are "active" for which sample: 
...
```
patient 164 is the baseline for the linear model and thus isn't a variable. If we have a look at the third line, we see that the variables "nI" as well as "i_CD183" are active. We now generate the variable coefficients `crat_coef` by fitting a linear model: 
...
In order to now obtain the expression values fitted by the model, we multiplicate the design matrix with this vector: 
```{r obtainFitCRAT2}
...
This way, for every row (i.e., sample) only the variables with a 1 in the design matrix are taken into account for calculating the fitted expression value. Two variables are selected for each row: one of "I" or "nI", and one sample variable. Note that this way the difference of the fitted expression values between inflamed-- and non--inflamed samples of one patient always are the same for all patients and is determined by the difference between the variable coefficients of `I` (6.77) and `nI` (7.19), which is -0.425. Note that this "difference" actually is a log fold change, as our expression data is on the log scale. -0.425 therefore is our log fold change. The same sample variable is turned on for both samples from one patient, and thus only corrects the fitted expression values towards the original ones measured from the patients: 
...
If we hadn't blocked for individuals, all of these lines would have collapsed to one single fitted line with a much higher variance.

In order to test whether the gene is differentially expressed or not, a t--test with the hypothesis that the difference between non--inflamed and inflamed tissue is zero. The t--test value depends on the variance of the sample: 
...
```{r tTest}
y1 <- na.exclude(crat_data$org_value[tissue=="nI"])
y2 <- na.exclude(crat_data$org_value[tissue=="I"])
res_t <- t.test(y1,y2,paired=TRUE)
...
interest. We can fit the linear model and compute the moderated \(t\)--statistics by calling the ` eBayes ` function.
...
As a diagnostic check, we also plot the  p--value histogram: We expect a uniform distribution for the
 p--values that correspond to true null hypotheses, while the a peak near zero shows an enrichment for low p--values corresponding to differentially expressed (DE) genes.  
...
We find 415 (CD) and 947 (UC) differentially expressed genes ("DE--genes").
...
For the actual testing, we use the `r Biocpkg("topGO") ` which
...
The test uses a 0.01 p--value cutoff by default. <!-- SR: is that true? look into Alexa 2006 paper, it says "this threshold is set to be 0.01 divided by the number of nodes in the GO graph with at least one annotated gene. This corresponds to a Bonferronie adjustment of the p-values" -->
...
edge--thickness (that is the line connecting two pathways) is proportional to the number of overlapping genes between
two pathways.
...
using downloaded KEGG data. <!-- SR: explain what KEGG is --> Furthermore, the package `r Biocpkg("EnrichmentBrowser")`
