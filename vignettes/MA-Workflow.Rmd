...
for (i in 1:length(Biobase::pData(palmieri_eset)$Factor.Value.phenotype.)){
  phenotype_name <- toString(Biobase::pData(palmieri_eset)$Factor.Value.phenotype.[i])
  phenotype_names <- c(phenotype_names, phenotype_name)
  disease_name <- toString(Biobase::pData(palmieri_eset)$Factor.Value.disease.[i])
...
pheatmap(dists, col = (hmcol),clustering_distance_rows = "manhattan",
                                 clustering_distance_cols = "manhattan")
         
```
#Alternative Heatmap with rownames indicating disease and phenotype
```{r PCAMetricsCalibrated_2, fig.height = 8.5, eval = TRUE }
dists <- as.matrix(dist(t(exp_palmieri), method = "manhattan"))
colnames(dists) <- NULL 
diag(dists) <- NA
...
pheatmap(dists, col = (hmcol),clustering_distance_rows = "manhattan",
                                 clustering_distance_cols = "manhattan",
         legend_breaks=c(min(dists), max(dists)), legend_labels=c("small difference", "big difference"))
         
```


#Alternative Heatmap without "manhattan" as distance measure between rows and columns; also low and high difference indicated in legend 
```{r PCAMetricsCalibrated_2, fig.height = 8.5, eval = TRUE }
dists <- as.matrix(dist(t(exp_palmieri), method = "manhattan"))
colnames(dists) <- NULL
diag(dists) <- NA
...
probe_stats <- filter(summarize(group_by(anno_palmieri, PROBEID), no_of_matches = n_distinct(anno_palmieri$SYMBOL)), no_of_matches > 1)
