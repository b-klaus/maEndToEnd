...
```{r RMAcalibration for RLE, eval=TRUE}
...
log_assay_data <- exprs(palmieri_eset)
...
RLE_data_trafo.m <- plyr::rename(RLE_data_trafo.m, c("value"="log2_expression"))
ggplot(RLE_data_trafo.m, aes (patient_probe, log2_expression)) + geom_boxplot(outlier.shape=NA) + ylim(c(-2, 2)) + theme(axis.text.x= element_text(colour="aquamarine4", angle=60, size=6.5, hjust=1, face="bold"))

#https://stackoverflow.com/questions/8510870/boxplot-from-row-values-in-a-dataframe
```


```{r RMAcalibration WITH normalization, eval=TRUE}
...
```
```{r alternative_heatmap_with_patient_probes_as_labels, fig.height = 8.5, eval = TRUE}
