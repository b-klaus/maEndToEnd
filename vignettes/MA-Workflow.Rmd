...
compares inflamed and non--inflamed colon tissue in two disease subtypes. For each disease, the differential gene expression between inflamed-- and non--inflamed colon tissue was analyzed. 
...
a paper studying the differences in gene expression in inflamed-- and non--inflamed tissue. 
...
Our aim is to analyze differential expression (DE) between the tissues in the two  diseases. As a proof of concept, we compare our results to the one found in the paper. 
...
It is saved in the `raw_data_dir` created above. The names of the downloaded files are returned as a list.
...
We now import the SDRF file from the raw data folder in order to obtain the sample annotation. 
...
SDRF2 <- read.delim(sdrf_location)

rownames(SDRF2) <- SDRF$Array.Data.File
SDRF2 <- AnnotatedDataFrame(SDRF2)
```
...
In the first step of the analysis, we create the Expression Set "raw_data", which contains array data, pheno data as well as the information of the correct chip annotation package to use:
...
We use the function ` read.celfiles`  from the `r Biocpkg("oligo") ` [@oligo] to import the files. This automatically creates an ExpressionSet, fills the sections "array data" with the data from the CEL files and uses the correct chip annotation package, 
...
raw_data <- oligo::read.celfiles(filenames = file.path(raw_data_dir, 
                                                SDRF2$Array.Data.File),
                         verbose = FALSE, phenoData = SDRF2)
...
RLE_data_subset <- (RLE_data[sample(nrow(RLE_data), 5000),])
...
ggplot(RLE_data_subset_trafo.m, aes (group, value)) + geom_boxplot()
...
by "transcript clusters". Hence the appropriate annotation package is called
...
```{r boxplot data for RLE_after RMA}

log_assay_data <- log2(exprs(palmieri_eset))
...
ggplot(RLE_data_trafo.m, aes (group, value)) + geom_boxplot(outlier.size=0.2) +
  ylim(c(-0.75, 0.75))

#https://stackoverflow.com/questions/8510870/boxplot-from-row-values-in-a-dataframe
```

```{r boxplot for non-inflamed tissues}
RLE_data <- as.data.frame(RLE_data)
# RLE_data_noninfl <- dplyr::select(RLE_data, contains("_I_"))) # doesn't work, because the files aren't named uniformly; sometimes, they are named I/II, sometimes A/B

RLE_data_noninfl <- RLE_data[,seq(1, ncol(RLE_data),2)]
RLE_data_noninfl_trafo <- as.data.frame(t(RLE_data_noninfl))
RLE_data_noninfl_trafo$group <- rownames(RLE_data_noninfl_trafo)
RLE_data_noninfl_trafo.m <- melt(RLE_data_noninfl_trafo, id.vars= "group")
ggplot(RLE_data_noninfl_trafo.m, aes (group, value)) + geom_boxplot(outlier.size=0.2) +
  ylim(c(-0.75, 0.75))
```
...
changed and we choose the Manhattan distance here (it uses absolute instead of
squared distances), as it is more robust. We set the diagonal of the distance matrix to ` NA ` in order to increase the contrast of the color coding. Those diagonal entries do 
...
```{r rownames for heatmap, fig.height = 8.5, eval = TRUE }
phenotype_names <- c()
disease_names <- c()

for (i in 1:length(Biobase::pData(palmieri_eset)$Factor.Value.phenotype.)){
  if(grepl ("non", toString(Biobase::pData(palmieri_eset)$Factor.Value.phenotype.[i])))
    {phenotype_name="non-infl."}
  else {phenotype_name = "infl."}
  phenotype_names <- c(phenotype_names, phenotype_name)
  
  if (grepl ("Crohn", toString(Biobase::pData(palmieri_eset)$Factor.Value.disease.[i])))
    {disease_name="CD"}
...
#Alternative Heatmap without "manhattan" as distance measure between rows and columns; also low and high difference indicated in legend; row names indicate tissue and disease, abbreviated 
...
In order to infer a cutoff from the data, we inspect the histogram of the median intensities. We visually set a cutoff line to the left of the histogram peak as `man_threshold`, in order to not exclude too many genes.



```{r alternative intensityBasedFiltering manually filtered}
...
abline(v=man_threshold, col="coral3", lwd=2)


samples_cutoff <- min(no_of_samples)


idx_man_threshold <- apply(exprs(palmieri_eset), 1, function(x){
...
We also have to exclude the PROBEIDs we have just excluded from the feature data we have generated before in `anno_palmieri`. We name this new feature data `anno_palmieri_new`, and we get it by left-joining `fData(palmieri_final)`and `anno_palmieri`. 
A left-join keeps the rows and columns of the first argument and adds the columns of the second argument.
Recall that fData enables us to access the feature data of an expression set.  
We add the column "PROBEID" to the feature data fData(palmieri_final). By left-joining with anno_palmieri, we add the "SYMBOL" and "GENENAME" columns from anno_palmieri, and thus get the feature Data for the filtered probes.




```{r excludeMultipleMappings, dependson="multipleMappings", cache=FALSE}
...
individual <- as.character(Biobase::pData(test)$Characteristics.individual.)

tissue <- str_replace_all(Biobase::pData(test)$Factor.Value.phenotype., " ", "_")
tissue <- ifelse(tissue == "non-inflamed_colonic_mucosa", "nI", "I")

disease <- str_replace_all(Biobase::pData(test)$Factor.Value.disease., " ", "_")
...
palmieri_fit_CD <- eBayes(contrasts.fit(lmFit(test[,disease == "CD"],
                                design = design_palmieri_CD),
                                contrast_matrix_CD))

contrast_matrix_UC <- makeContrasts(I-nI, levels = design_palmieri_UC)

palmieri_fit_UC <- eBayes(contrasts.fit(lmFit(test[,disease == "UC"],
...
back_genes_idx <- genefinder(test, as.character(DE_genes_CD), 
                       method="manhattan", scale="none")

back_genes_idx <- sapply(back_genes_idx, function(x)x$indices)

back_genes <-featureNames(test)[back_genes_idx]
...
```{r runReactomeEnrichment, dependson="mapIDsToEntrez", eval = TRUE, cache=FALSE}
