...
We use a custom function here for now, `https_getAE` , since EBI
has recently switched to https and `getAE` only supports http.
See als <https://www.ebi.ac.uk/services>.

```{r my_getAE, echo=TRUE}
...
anno_AE <- https_getAE("E-MTAB-2967", path=raw_data_dir, type="raw")
```
...
```{r}
#SR: suggestion for annotation and filtering, in my opinion easily understandable 
anno_palmieri <- anno_palmieri[complete.cases(anno_palmieri$SYMBOL),] #filter out NA values 

anno_palmieri <-  #filter out duplicated values; note, however, that it keeps the first of each type 
...
dim(probe_stats)
```
#alternative way of writing the %>%-function above; doesn't work!!!!
```{r multipleMappings_alternative_writing, dependson="annotateData"}
...
probe_stats <- filter(summarize(group_by(anno_palmieri, PROBEID), no_of_matches = n_distinct(SYMBOL)), no_of_matches > 1)
...
#Code suggested by Bernd (18.04.): mit Annotate_eset-funktion aus affycoretools
```{r testAnnotateFunction}

source("https://bioconductor.org/biocLite.R")
biocLite("affycoretools") #download package
# annotateEset(object, x,
#   columns = c("PROBEID", "ENTREZID", "SYMBOL", "GENENAME"),
#   multivals = "first")

library("affycoretools")
test <- annotateEset(palmieri_filtered, hugene10sttranscriptcluster.db,
                     columns=c("PROBEID", "ENTREZID", "SYMBOL", "GENENAME"),
                     multivals = "first") #note: multivals = "filter", which would be the desired output, is not supported by annotateEset --> find out why! 

test <- test[complete.cases(test@featureData@data),] #filter out NA values 
...
```{r annotateEset method modified}
#doesn't work
...
test2 <- annotateEset(palmieri_filtered, hugene10sttranscriptcluster.db,
                     multivals = "filter")
```
...
ids_to_exlude <- ((featureNames(palmieri_manfiltered) %in% probe_stats$PROBEID) |
               featureNames(palmieri_manfiltered)  %in% subset(anno_palmieri,
...
# match_id_in_anno <- match(fData(palmieri_final)$PROBEID, anno_palmieri$PROBEID)
# fData(palmieri_final) <-  cbind(fData(palmieri_final) anno_palmieri[match_id_in_anno,]
# stopifnot(all(rownames(fData(palmieri_final)) == fData(palmieri_final)$PROBEID))

fData(palmieri_final) <- left_join(fData(palmieri_final), anno_palmieri)

#leftjoin besser erklären!
