...
# base and recommended pkgs  
pkgs_to_keep <- ifelse(!is.na(inst_pkgs$Priority), TRUE, FALSE)
pkgs_to_remove <- rownames(inst_pkgs[!pkgs_to_keep, ])
...
The paired t-test has computes the variance \(s^2\)   from the paired differences.
This is lower than the variance of a standard t--test and thus higher power
as long as the expression values for the same individual are correlated (
[see e.g. the article on Wikipedia](https://en.wikipedia.org/wiki/Paired_difference_test)).
...
We get a low p--value of `r round(res_t$p.value, 6)` and thus can conclude
