...
```{r createFactors, eval=TRUE, dependson="excludeMultipleMappings" }
...
We will illustrate this below using actual numbers. 
...
interest. 
We can fit the linear model and compute the moderated \(t\)--statistics
by calling the ` eBayes ` function.
...
shows an enrichment for low p--values corresponding to differentially expressed (DE)
genes.  
...
#SR: comment on the aj.P.val or don't include it at all; why do we set it to 0.05? In the text below, it is mentioned that in CD, a p value of 0.001 corresponds to an adjusted p value of 0.05 in CD; however, it is also mentioned that for UC it would be 0.02, and still 0.05 is used in the UC chunk
```
...
with a p--value less than 0.001. The corresponding FDRs are also listed in the results file and are 0.05 in Crohn's
...
back_genes <- setdiff(back_genes, DE_genes_CD) #SR: so that we don't have any of the differentially expressed genes in the background

    
intersect(back_genes, DE_genes_CD) #SR: nur zur Überprüfung ob da noch eine Überschneidung vorhanden ist, oder?
length(back_genes)
...
c) We name the vector elements with the corresponding `gene_IDs`and each element is a 0 if the corresponding gene is a background gene and a 1 if the corresponding gene is a DE--gene.

```{r createFactorOfInterestingGenes, dependson="GOAnalysisCreateBackgrounds", eval=TRUE}
gene_IDs <- rownames(table_CD)
in_universe <- gene_IDs %in% c(DE_genes_CD ,  back_genes) #SR: gives out TRUE/FALSE for each element in gene_ID
...
gives genes annotated to them; the significant ones are denoted with a "2" in the "raw p-value" column, the not--significant ones with a "1". We therefore select `raw p-value == 2`. 
...
#bedeutet raw p-value == 2, dass das die Gruppe der p-values ist, die signifikant sind? --> 2 sind die differential expressed genes, 1 die nicht; aber ich verstehe immer noch nicht, warum wir keine normalen p-values bekommen
...
<!-- SR: why do we take Fisher classic here for the three most significant nodes?; Bernd: man könnte auch elim nehmen, aber typischerweise wird der Standardfischertest genommen -->
...
entrez_ids2 <- mapIds(hugene10sttranscriptcluster.db, 
      keys = table_CD$PROBEID, 
      keytype="PROBEID",
      column = "ENTREZID")
#SR: discuss: what das keytype = "PROBEID" do? --> erklärt bei annotation workflow auf bioconductor in help-section 
```


We can now run the enrichment analysis that performs a statistical test
based on the hypergeoemtric distribution that is the same as a one sided Fisher--test, <!-- SR: for more than two dimensions, right? -->
...
<!--SR: discuss with Bernd: what was it that you wanted to implement right in the beginning and we took it out because the max amount of DLLs was reached? It had to do with reactome and enrichment analysis, right? -> stellen wir erst noch hinten an-->
...
<!-- SR: edge thickness refers to the thickness of the lines between two pathways? and size refers to the number of genes participating in one pathway? -->
...
#it was suggested to use emapplot bcs enrichMap will be removed in the next version 
```
