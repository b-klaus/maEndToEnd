...
RLE_data_gathered <- gather(RLE_data, patient_probe, log2_expression_variation)
...
When looking at the boxplot in (\@ref(fig:boxplotDataForRLETidy), 5 arrays could be considered to be outliers: 2826_I, 2826_II, 3262_II, 3302_II and 3332_II are negatively y--shifted. We will keep these samples in mind for heatmap cluster analysis later on in the workflow. Arrays that are confirmed to be outliers by heatmap analysis can be removed for subsequent analysis.
...
On the heatmap plot ((\@ref(fig:clusteringHeatmap)) we also see that the samples do not cluster strongly by tissue, confirming the impression from the PCA plot that the separation
...
The outliers that could be found here are 2826_II, 3262_II, 3271_I, 2978_II and 3332_II ((\@ref(fig:alternativeHeatmapWithPatientProbesAsLabels)). 
...
In a second step, we directly filter out the probes that don't map to a gene, i.e. that don't have a gene symbol assigned. 
...
Now, we filter for PROBEIDs with multiple matches, i.e. when `no_of_matches > 1`.
...
diseases to UC and DC and the names of the tissue types to nI and I, respectively. 
...
We see that overall, this gene is expressed lower in inflamed tissue ((see \@ref(fig:visualizeExpressionChanges)). We also see that the absolute expression intensities vary a lot between patients; if we hadn't blocked for individuals, the linear model would fit one single line to all of these lines would collapse to one single fitted line, resulting in a very high variance.
However, we have already taken care of this problem by introducing blocking factors on the individuals, which allows us to fit one line to each individual.
...
Note that the differences of the fitted expression values between inflamed-- and non--inflamed samples of one patient always are the same for all patients and is determined by the difference between the variable coefficients of `I` (6.77) and `nI` (7.19), which is -0.425 ((see \@ref(fig:3obtainFitCRAT)). This is the case because the same sample variable is activated by the design matrix for both samples from one patient, and thus only corrects the fitted expression values towards the original ones measured from the patients. 
...
Results can be extracted by use of the `topTable` function. We extract the results for both Crohn's disease as well as ulcerative colitis, and the results are sorted by their absolute \(t\)--statistics.
As a diagnostic check, we also plot the p--value histogram: We expect a uniform distribution for the
 p--values that correspond to true null hypotheses, while a peak near zero shows an enrichment for low p--values corresponding to differentially expressed (DE) genes.
...
When comparing the "background gene" curve to the "foreground gene" curve, we see a similar curve form, indicating a sensible background matching ((see \@ref(fig:multidensityPlot)). Note that the right-shift of the "foreground-gene" curve in comparison to the "background-gene" curve indicates that DE--genes are generally very highly expressed, so that it wasn't possible to find background--genes with an equally high expression.
...
For the actual testing which GO gene sets are enriched in inflamed tissue, we use the `r Biocpkg("topGO") ` which implements a nice interface to Fisher testing and also has additional algorithms
...
1) we create an `in_universe`vector by using the `%in%`matching function. We want to know which elements from `gene_IDs` are also contained in `DE_genes_CD` and `back_genes`, as the latter two are our gene universe we use for enrichment analysis. We get a vector with the length of `gene_IDs` that has the entry `TRUE`when the corresponding gene in `gene_IDs`could be also found in `DE_genes_CD`or `back_genes`and `FALSE`otherwise.
...
b) Then, we convert the elements in `all_genes` from `TRUE`and `FALSE` to 0 and 1 by converting the vector to an integer vector. Also, we convert the vector to a factor. <!-- SR: why do we do that? in the vignette description it says that it could also be a numeric vector -->
c) We name the vector elements with the corresponding `gene_IDs`, and each element in the vector is a 0 if the corresponding gene is a background gene and a 1 if the corresponding gene is a DE--gene.
...
The test uses a 0.01 p--value cutoff by default. <!-- SR: is that true? look into Alexa 2006 paper, it says "this threshold is set to be 0.01 divided by the number of nodes in the GO graph with at least one annotated gene. This corresponds to a Bonferroni adjustment of the p-values" -->
...
with a p--value below the specified cutoff ((see \@ref(fig:reactomeBar)).
