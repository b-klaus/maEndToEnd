...
ggplot2::ggplot(RLE_data_gathered, aes(patient_probe,
                                       log2_expression_variation)) + 
...
## Multiple testing and comparison to the paper results
...
rate** or FDR for short. As we have seen, the FDR  of our genes list
...
the table is `r max(subset(table_CD, P.Value < 0.001)$adj.P.Val)`,
which is an order of magnitude lower than the rate we can infer 
from p--values alone.
...
corresponding FDRs (just as we did for the CD data here).

For a p--value cutoff of 0.001. The corresponding FDRs are 0.05 in Crohn's disease and 0.02 in ulcerative colitis. There are four tables in total, giving
the list of up and downregulated genes in CD and UC, respectively.
We calculate the overlap between our results and the ones from the paper as the ratio of the genes that were found in both analyses and the genes that were only found in the paper.
