...
by t--test, we will pick the gene with the PROBEID 8164535. It the gene symbol 
...
baseline expression levels, this would lead to a very variance of
...
For example, the fitted value for patient sample `2114_I.CEL` is 6.97: it is the sum of the respective activated variable coefficients "nI" (7.1917 ) and "i_CD2114" 
(- 0.2215). 
...
Note that the differences of the fitted expression values between inflamed-- and non--inflamed samples of one patient is the same for all patients and is determined by the difference between the variable coefficients of `I` (6.77) and `nI` (7.19), which is - 0.425 ((see Figure \@ref(fig:3obtainFitCRAT)). 
...
The "difference" between non--inflamed and inflamed tissue of - 0.425 actually 
is a log fold change, as our expression data is on the log scale. - 0.425 
...
The paired t-test has lower variance \(s^2\)  and thus higher power
compared to a standard t--test as long as the expression values 
for the same individual are correlated ([see the article on] wikipedia(https://en.wikipedia.org/wiki/Paired_difference_test)).

We thus have improved the linear--model \(t\)--test  
...
We get a low p--value of `r res_t$p.value` and thus have found that the *CRAT* gene is differentially expressed between non--inflamed and inflamed tissue.
