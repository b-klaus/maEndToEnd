...
We specifiy our `AnnotatedDataFrame` created earlier as `phenoData`. We then 
...
rawData <- read.celfiles(filenames = file.path(rawDataDir, SDRF$Array.Data.File),
...
```{r array_Quality_metrics_calibrated, eval=FALSE}
...
In our case this would be `table(pData(palmieriEset)$Factor.Value.disease.)[1]`.
...
and $( \mathbf{X}^T \mathbf{X})^{-1} = \text{Diag}(c_1, \dotsc c_p) $ is a diagonal matrix
with some values $c_j$.
This is commonly the case for a design with no intercept. Note that the
covariates are modeled
as fixed quantities, and are not  centered so $\mathbf{X}^T \mathbf{X}$ is not actually a
covariance  matrix. In this case the estimate of the coefficient vector becomes

$$
\hat \beta =  \text{Diag}(c_1, \dotsc c_p)  \mathbf{X}^TY = \bigl(X_1, \dotsc, X_p\bigr)^TY
$$
...
$\beta_1 - \beta_2 = 0$, $\beta_1 - \beta_2 - 2\beta_3 = 0$ or something like that the test statistic
...
We can use the function `glht` from the `r CRANpkg("multcomp") `
...
PalmieriDERes <- sapply(1:4, function(i) read.xlsx(cols = 1,
"PalmieriDERes.xlsx", sheet = i, startRow = 4))
...
We see that we get a moderate overlap of ` r overlapCD ` for CD and
` r overlapUC ` for UC. Note that is recommendend to always to choose an
...
The function ` genefinder ` from the `r Biocpkg("genefilter") ` will be used
...
DEgenesCD <- subset(tableC, adj.P.Val < 0.1)$PROBEID

backGIdx <- genefinder(PalmieriFinal, as.character(DEgenesCD), 
...
```{r createTopGODataSet, dependson="createFactorOfInterestingGenes", eval=TRUE }
ont <- "BP"
...
```{r runtopGOTests, results='hide', eval=TRUE, dependson = "createTopGODataSet"}
...
showSigOfNodes(tgd, score(resultTopGOElim), firstSigNodes = 3,
               useInfo = 'def')
```
...
of specific pathways using the free, open-source, curated and peer reviewed pathway  reactome pathway database. The package requires entrez IDs, so
we convert out PROBEIDs to entrez IDs using the function `mapIDs` from the package `r Biocpkg("AnnotationDBI")`. This will create a named vector that
maps the PROBEIDs to the entrez ones.


```{r mapIDsToEntrez, dependson="createFactorOfInterestingGenes"}

entrezIDs <- mapIds(hugene10sttranscriptcluster.db, 
      keys = rownames(tableC), 
       keytype="PROBEID",
       column = "ENTREZID")
...
based on the hypergeoemtric distribution that is the same as one sided Fisher--Test, which `r Biocpkg("topGO")` calls "Fisher--classic".
Details can be found in the vignette of the `r Biocpkg("DOSE")` package.

```{r runReactomeEnrichment, dependson="mapIDsToEntrez"}
reactomeEnrich <- enrichPathway(gene = entrezIDs[DEgenesCD], 
                                universe = entrezIDs[c(DEgenesCD, backG)],
                                organism = "human",
                                pvalueCutoff = 0.05,
                                qvalueCutoff = 0.9, 
                                readable = TRUE)

# trim pathway names to 20 characters

reactomeEnrich@result$Description <- paste0(str_sub(reactomeEnrich@result$Description, 1, 20), "...")

head(summary(reactomeEnrich))[1:6]
```
...
The top pathways can be displayed as a bar char that displays all catergories
with a p--value below the specified cutoff.

```{r reactomeBar, dependson="runReactomeEnrichment"}
barplot(reactomeEnrich)
```
...
```{r enrichMap, dependson="runReactomeEnrichment", fig.height = 6}
enrichMap(reactomeEnrich, n = 20)
```


The package `r Biocpkg("clusterProfiler") ` can also perform these analyses 
using downloaded KEGG data. Furthermore, the package `r Biocpkg("enrichmentBrowser") ` additionally offers network--based enrichment analysis of indidividual 
