...
We walk through an end-to-end Affymetrix microarray differential
...
but can easily adapted to other similar platforms.
The data is a typical clinical microarray data set that compares inflammed and
non--inflammed colon tissue in two diseases.
...
analysis. As experimental designs can be complex, a self contained introduction to linear models
...
in raw data files ([Data from Palmieri et. al. on ArrayEpress](https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-2967/)).
...
We now  import the SDRF file directly 
...
involving Bioconductor packages. In particular, the rownames of the `phenoData`
...
row names of the `featureData` (as they represent the feature IDs). This is 
...
estimated probe intensity values. Each CEL file contains data on the intensity of each
probe on the chip, as well as some other quantities.
We collect the information about the CEL files and import and them into the 
variable `raw_data`.

The function ` read.celfiles `
from the `r Biocpkg("oligo") ` [@oligo] can be
used to import the files. The package automatically uses
`r  Biocannopkg("pd.hugene.1.0.st") ` as the chip annotation package as the 
...
table --- to enforce this,
this we use the column `Array.Data.File` of the `SDRF` table as the `filenames` 
argument.
...
related to the experimental factors of interest (IDs of the individuals, disease 
...
e.g. by the experimental conditions.
...
Gene arrays, a lot of the probesets on the "Gene" arrays are made up of 3 
...
although strictly speaking RMA is merely a summarization method [@Irizarry_2003, @Bolstad_2003, @Irizarry_2003a].
...
uses the versatile called quantile normalization method  that
...
in the way \(B\) is removed and an estimate of \(theta\) is obtained.
...
default. The default distance  is the Euclidean one, however this can of course be
...
package produces reports that compute some metrics that can be used for outlier 
removal.
...
as differentially
expressed although they are barely above the "detection" limit 
and not very informative in general. We will perform
...
across genes for the variance estimation leading to a "moderated" \(t--statistics\), 
and  making the analyses stable even for experiments 
with a small number of arrays [@Smyth_2004].
...
pathway  reactome pathway database [@Croft_2013, @Fabregat_2015]. The package 
requires entrez IDs, so we convert our PROBEIDs (trancript cluster identifiers) 
to entrez IDs using the function `mapIDs` from  the package `r Biocpkg("AnnotationDbi")`. 
