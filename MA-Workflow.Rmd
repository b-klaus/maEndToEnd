...
#sed 's/.png/.pdf/g' | \
sed 's/MA-Workflow_files\/figure-latex/figure-latex/g' > MA-Workflow_clean.tex
...
have to worry about the details. However, some contrains have to be met. 
...
no_of_samples <- table(pData(palmieri_eset)$Factor.Value.disease.)[1]

idx_thresh_median <- apply(exprs(palmieri_eset), 1, function(x){
                                sum(x > thresh_median) >= no_of_samples})
...
in more complex models. We canspecify a more complex design pretty easily: 
...
We can now insepect the results.  We look at the top 100 GO categories according
