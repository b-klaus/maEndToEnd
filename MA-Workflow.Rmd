...
opts_chunk$set(echo=TRUE,tidy=FALSE,include=TRUE,
               dev=c('png'), fig.width = 6, fig.height = 3.5,
               comment = '  ', dpi = 300,
cache = TRUE)
```
...
The data is a typical clinical microarray data set that compares inflammed and
normal colon tissue in two diseases.
...
colonic mucosa tissue was obtained via a biopsy. We analyze differential 
expression (DE) between the tissues in the two  diseases separately.
...
and has the accession code **E-MTAB-2967**. 

Each ArrayExpress data set has a landing page summarizing the data set, ([Data from Palmieri et. al. on ArrayEpress](https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-2967/))
...
the first place, because it will be the starting point for many  analyses
involving Bioconductor packages.
...
We collect the information about the CEL files and import and then save them in
the variable `rawData`.
...
related to the experimental factors (IDs of the individuals, disease 
of the individual and the mucosa type).
...
The PCA plot of the raw data shows that the first principal
...
in the optical detection system. Observed intensities need to be adjusted to 
...
chips, which usually represent a gene/transcript as mentioned above. Furthermore, there
...
Gene arrays, a lot of the probesets on the Gene ST arrays are made up of 3 
or fewer probes. Thus, a summarization on the probeset / Exon level 
is not recommended for Gene arrays but is still possible by using the 
...
default option of which is "core", which uses transcript clusters containing
...
uses a "strong" normalization method called quantile normalization that
...
shifting and scaling log--scale intensity values within arrays [@vsn].
...
heatmap of the sample--to--sample distances, we first compute the distances using
...
to increase the contrast. Those diagonal entries do not contain information since the
...
The second PC roughly separates Chron's disease from ulcerative colitis, while the first 
...
package produces reports than compute some metrics that can be used for outlier 
removal.


## Filtering based on intensity 
We now filter out lowly expressed genes. Microarray data commonly show a large number
of probes in the background intensity range. They do not change much and have a low
...
set of probes in practice. In the histogram, we can clearly see an enrichment
of low medians on the left hand side. These are the genes we want to filter. 

In order to do this, we inspect the histogram of the median--intensities. We
visually fit a central distribution \(0.5 \cdot N(5.1, 1.18)\) to the probe--wise 
medians, which represents their typical behavior in the data set at hand.
...
We can also fit a linear model without an intercept is fitted
...
the tissue types. We simplify the names of the
...
We  now fit the linear model and
...
Thus, we create a contrast matrix consisting of one line. `r Biocpkg("limma")` 's function
...
collection of functional gene sets [@Ashburner_2000, @GO_2015]. 
For a nice introduction to the GO see [@du_Plessis_2011].
...
pathway  reactome pathway database [@Croft_2013, @Fabregat_2015]. The package requires entrez IDs, so
...
correspondance about the annotation resources available
