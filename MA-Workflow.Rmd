...
In this document we introduce a complete workflow for a typical microarray
...
The data used is from  [Palmieri et. al.](http://dx.doi.org/10.1097/MIB.0000000000000370),
...
# Download files using ArrayExpress

The data has the accession number **E-MTAB-2967**
...
# Get the annotation for the data 

In the repository, for each dataset, ArrayExpress stores a 
MAGE--TAB document with standardized format. A MAGE-TAB document contains 
up to five different types of files
...
`r Biocpkg("Biobase")` expression set which we will cover briefly next.
...
` AnnotatedDataFrame ` from the `r Biocpkg("Biobase") ` package.
...
the `r Biocpkg("limma") ` although a variance based filter should filter a similar
...
###*{Exercise: Filtering based on variance}
