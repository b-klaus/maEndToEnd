...
% graphics.off();rm(list=ls());library('knitr');knitr::knit2pdf('MA-Workflow.Rnw');
...
<<GOAnalysisCreateBackgrounds, eval=TRUE, dependson="extractResults", warning=FALSE>>=
...
<<createTopGODataSet, eval = TRUE, dependson="createFactorOfInterestingGenes" >>=
ont <- "BP"
...
<<runtopGOTests, results='hide', eval=TRUE, dependson = "createTopGODataSet">>=
