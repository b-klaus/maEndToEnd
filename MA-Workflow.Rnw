...
% graphics.off();rm(list=ls());knitr::knit2pdf('MA-Workflow.Rnw');
...
\title{Differential expression analysis of microrarray data / analysis of  other
...
In this document we introduce a complete sample workflow for a typical microarray
analysis. Data import, preprocessing, differential expression  and
enrichment analysis are discussed. We also introduce the necessary mathematical
background along the way. We re--analyze a typical clincal data set.

The data is from  \href{http://dx.doi.org/10.1097/MIB.0000000000000370}{Palmieri et. al.},
a paper studying the differences between inflamed and noninflamed
colonic mucosa between patients suffering
from Ulcerative colitis (UC) or Crohn's disease (CD). There are 15 CD and
14 UC patients.


\section{Download files using \Biocpkg{ArrayExpress}}

The data has the accession number \textbf{E-MTAB-2967}
and the landing page is here:

\href{https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-2967/}{Palmieri et. al. at ArrayEpress}

We use the \Biocpkg{ArrayExpress} Bioconductor package to obtain the links to
download the raw data.

\section{Get the annotation for the data}
...
the information on the samples. The following code downloads the raw
data and extracts it. Subsequently it will be summarized in a
\Biocpkg{Biobase} expression set which we will cover briefly next.

\subsection{Bioconductor \Robject{ExpressionSets}}
...
You can use the functions \Rfunction{pData} and \Rfunction{fData} to extract
the sample and feature annotation respectively from an \Robject{ExpressionSet}.
The function \Rfunction{exprs} will return the expression data itself as a
matrix.

\section{Get raw andannotation data}

Wih the un--evaluated code below, we download the raw data from
\href{https://www.ebi.ac.uk/arrayexpress/}{ArrayExpress}.

It is saved in the directory \textbf{rawDataDir} which defaults to the current
working directory. The names of the downloaded files are returned as a list.

<<getDataEBI, eval=TRUE >>=
...
\Rclass{AnnotatedDataFrame} from the \Biocpkg{Biobase} package.
...
\subsection{Import of microarray data and initial quality control}

\subsubsection{Importing CEL files}
...
we want to import and then save them in  the variable \Robject{rawData}.

The followng code results in a character vector with the paths to the
CEL files we want to import. the function \Rfunction{read.celfiles}
from the \Biocpkg{oligo} can be
used to import the files. The package automatically usees
\Biocannopkg{pd.hugene.1.0.st} as the chip annotation package.

We specifiy our \Rclass{AnnotatedDataFrame}

<<importCelfiles, results='hide', eval=TRUE, dependson="getSDRF">>=
...
We inspect the raw data and remove some uninteresting phenoData columns and retain
only those that are related to the experimental factors. (IDs of the individuals,
disease of the individual and the mucosa type)

<<inspectPhenoData, results='hide', eval=TRUE >>=
#load(file.path("dataMAWorkflow","rawData.RData"))
head(pData(rawData))
head(exprs(rawData))
...
\subsubsection{Quality control of the raw data}


The package \Biocpkg{arrayQualityMetrics} produces an html report, containing lots of
quality control plots together with a description of their
aims. We also check our raw data using this reporting tool.
Note taht it cannot use more than 10 phenoData columns.
...
The \Biocpkg{arrayQualityMetrics} report shows that the second principal
...
<<removeOutliers, dependson="inspectPhenoData">>=
...
\subsection{Background adjustment, calibration, summarization and annotation}


\subsubsection{Within--array normalization}
...
\subsubsection{Across--array normalization}
...
<<annotation data base content, eval = TRUE>>=
...
\subsubsection{Old and new ``probesets" of Affymetrix microarrays}
...
\subsubsection{One--go preprocessing in oligo}
\Biocpkg{oligo} allows us to perform background correction, normalization
...
<<RMAcalibration, eval=TRUE>>=
palmieriEset <- oligo::rma(rawData, target="core")
#save(palmieriEset, file = "palmieriEset.RData")
@

The parameter \Robject{target} defines the degree of summarization, the
default option ``core", which contains transcript clusters containing
``safely" annotated genes. Other options for \Robject{target} include ``extended"
...
An alternative to quantile normalization is the \Biocpkg{vsn} algorithm,
...
\subsubsection{Some mathematical background on normalization/calibration
...
and also the \Biocpkg{vsn} algorithm. They differ in the way $B$ is removed and
an estimate of $\vartheta$ is obtained.



\subsubsection{Quality assessment of the calibrated data}
After running the RMA algorithm it is a good idea to produce another
quality control report.

<<array Quality metrics calibrated, eval=FALSE>>=
...
the \Rfunction{dist} function. We need to transpose the expresssion values since
...
<<PCAMetricsCalibrated, fig.height = 4.5, fig.cap="PCA plot and clustering of the calibrated data", eval = TRUE >>=
#load(file.path("dataMAWorkflow","palmieriEset.RData"))
...
\subsubsection{Filtering based on intensity}
...
the \Biocpkg{limma} although a variance based filter should filter a similar
set of probes in practice. (see the exercise). We filter low gene
...
<<intensityBasedFiltering, fig.width=10, fig.height=6, eval=TRUE>>=
...
#save(PalmieriFiltered, file = "PalmieriFiltered.RData")
@




\subsubsection*{Exercise: Filtering based on variance}
In this exercise we filter the expression data based on the row--mad (mean
absolute deviation) instead of the row median. This
can be conveniently done using the function
\Rfunction{rowMads}.

\begin{enumerate}[label=(\emph{\alph*})]
\item A histogram of the square root of the mads will be
roughly symmetric. Fit a null model via visual inspection.
\item Use the 5\% quantile of this model as a threshold. Compare
the thresholded genes to the intensity based approach. Is the overlap
very large? (HINT use the function \Rfunction{table} to check this)
\end{enumerate}


\subsection{Annotation of the transcript clusters}
...
from \Biocpkg{AnnotationDbi} to query the gene symbols and associated
short descriptions for the transcript clusters.



<<annotateData, eval=TRUE, dependson="intensityBasedFiltering">>=
#load(file.path("dataMAWorkflow","PalmieriFiltered.RData"))
...
\subsection{Removing multiple mappings and building custom annotations}
...
# View(subset(annoPalmieri, PROBEID %in% probeStats$PROBEID))
...
<<excludeMultipleMappings, dependson="multipleMappings">>=
...
\href{https://dx.doi.org/10.1371/journal.pone.0139516}{Arloth et. al.}
...
In any case, the package \Biocpkg{pdInfoBuilder} can be used to build custom
annotation packages for use with \Biocpkg{oligo}. In order to do this,
...
\section{A short overview of linear models}
...
like \Biocpkg{limma},  \Biocpkg{DESeq2} and \Biocpkg{edgeR} are actually
...
the concepts better. The Khan Academy offers nice (and free) online courses:
...
\intertext{With $\bX$ being the so called \textbf{design matrix}}
\bX :=  \bigl( \mathbb{1}_n, X_1, \dotsc, X_p\bigr)
\intertext{$\mathbb{1}_n$ is a columns vector of ones called the \textbf{intercept} and }
X_p = (x_{p1}, \dotsc, x_{pn})^T
\intertext{is a column vector of measurements for covariate $p$. The regression
...
\intertext{leading to the estimate of the coefficient vector}
\hat \beta = ( \bX^T \bX)^{-1} \bX^TY
\intertext{An important special case is when the design is ``orthogonal", i.e.
...
\subsection{Creating design matrices in R}
...
\begin{itemize}
\item  \Rfunction{formula}
\item   \Rfunction{model.matrix}
\end{itemize}
...
\subsection{Singular model matrices}
...
\begin{gather*}
\colvec{8}{1}{1}{1}{1}{0}{0}{0}{0}
 = 1\cdot \colvec{8}{1}{1}{1}{1}{1}{1}{1}{1} -1\cdot \colvec{8}{0}{0}{0}{0}{1}{1}{1}{1}
\end{gather*}
...
<<singularmodel for factors data and fit>>=
...
<<singularmodel for real valued  data and fit>>=
...
\subsection{Using contrasts to test hypotheses  in linear regression}
...
from the \CRANpkg{DAAG}.
The data set \Robject{toycars} gives  the distance traveled by one of
...
\begin{itemize}
\item \Robject{angle}: Angle
\item \Robject{distance}: Traveled distance
\item \Robject{car}: Car number (1, 2 or 3)
\end{itemize}
...
<<toycars_linear_model >>=
lm.car <- lm(distance~angle+car, data = toycars)
summary(lm.car)
@


\subsection{Testing general linear hypotheses}
...
We can use the function \Rfunction{glht} from the \CRANpkg{multcomp}
...
contrasts of interest as we will see later when using \Biocpkg{limma} and in the
exercise below.
...
\subsubsection*{Exercise: Model for toycars without intercept}
In this exercise a linear model without an intercept is fitted
to the toycars data set.

\begin{enumerate}[label=(\emph{\alph*})]
\item Fit a linear model to the toycars data set without an intercept.
\item Test the between--group contrasts and compare the results to the ones
obtained with the model including an intercept introduced above. Do they
agree?
\end{enumerate}

\section{Linear models for microarrays}

We now apply linear models to microarrays. Specifically, we discuss
how to use the \Biocpkg{limma} for differential expression analysis.
...
\subsection{A linear model for the data}
...
\subsection{Contrasts and hypotheses tests}
...
We create a contrast matrix consisting of one line. \Biocpkg{limma}s function
...
<<createContrastMatrixAndFitModelFit, eval=TRUE, dependson="createDesign" >>=
...
and the moderated variance estimation that \Biocpkg{limma} performs.

<<extractResults, eval = TRUE, dependson="createContrastMatrixAndFitModelFit">>=
...
\subsection{Comparion to the paper results}
...
\href{http://links.lww.com/IBD/A795}{this link}. The paper results are  given
...
<<compareDEgenes, dependson="extractResults">>=
...
\section{Gene ontology enrichment analysis}
...
\subsubsection{Matching the background set}

The function \Rfunction{genefinder} from the \Biocpkg{genefilter} will be used
...
<<GOAnalysisCreateBackgrounds, eval=FALSE, dependson="extractResults", warning=FALSE>>=
DEgenesCD <- rownames(subset(tableC, adj.P.Val < 0.05))

backGIdx <- genefinder(PalmieriFinal, DEgenesCD,  5,
  method="manhattan", scale="none")
backG <-featureNames(PalmieriFinal)[sapply(backGIdx, function(x)x$indices)]
...
perform the actual testing. For this purpose we use the \Biocpkg{topGO} which
...
\subsubsection{Running topGO}

We first create a factor \Robject{allG} which indicates for every gene in
our background / universe, whether it is differentially expressed or not.

<<createFactorOfInterestingGenes, dependson="GOAnalysisCreateBackgrounds", eval=FALSE>>=
...
We now initialize the \Biocpkg{topGO} data set, using the GO annotations contained
in the annotation data base for the chip we are using. The \Robject{nodeSize}
...
<<createTopGODataSet, eval=FALSE, dependson="createFactorOfInterestingGenes" >>=
ont <- "BP"
...
Now the tests can be run. \Biocpkg{topGO} offers a wide range of options,
...
<<runtopGOTests, results='hide', eval=FALSE, dependson = "createTopGODataSet">>=
...
<<processtopGOResults, eval=FALSE, dependson="runtopGOTests">>=
...
vignette and the references therin and also GeneSetEnrichment workflow
in Bioconductor:

%

\begin{itemize}
  \item \url{http://bioconductor.org/packages/release/BiocViews.html#___GeneSetEnrichment}
\end{itemize}


\section{\Biocpkg{EnrichmentBrowser} based anlysis}

The {\Biocpkg{EnrichmentBrowser} package enables

<<prepareDataEB, dependson="createDesign">>=
#load(file.path("dataMAWorkflow", "PalmieriFinal.RData"))
...
agree?
\end{enumerate}
\subsubsection*{Solution: Model for toycars without intercept}


<<toycars-exercise, echo = TRUE, eval = TRUE, results='hide'>>=
# a
#######################################################
toycars$car = as.factor(toycars$car)
lm.car.noI <- lm(distance~0 + angle+car, data = toycars)
summary(lm.car.noI)
# b
#######################################################
C.noI <-  rbind(c(0,-1,1,0), c(0,-1,0,1), c(0,0, 1,-1) )
colnames(C.noI) <- rownames(summary(lm.car.noI)$coefficients)
rownames(C.noI) <- c("car2 - car1", "car3 - car1" , "car2 - car3" )
C.noI
summary(glht(lm.car.noI, C.noI), test = adjusted(type ="none"))
@
...
\subsubsection*{Solution: Filtering based on variance}

<<variance based filtering, results='hide', eval = FALSE>>=
# a
#######################################################
load(file.path("dataMAWorkflow", "TinaEXP.RData"))
TinaSqrtMads = sqrt(rowMads(exprs(TinaEset)))
tmp <- hist(TinaSqrtMads, 100, col="lavender", freq = FALSE)

emp.mu <- tmp$breaks[which.max(tmp$density)] + 0.005
emp.sd <- mad(TinaSqrtMads)/4
eta0 <- 0.40
lines(sort(TinaSqrtMads), eta0*dnorm(sort(TinaSqrtMads),
      	mean = emp.mu , sd = emp.sd ),
				col = "coral3", lwd =4)
# b
#######################################################

### find 5% quantile of fitted null model
thresh.mad <- qnorm(0.05 / eta0, emp.mu, emp.sd)
idx.thresh.mad <- TinaSqrtMads > thresh.mad
table( idx.thresh.mad )

## overlap
prop.table(table(idx.thresh.mad , idx.thresh.median ))
@
